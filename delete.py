...
def triangle(leg: float = 0.0) -> float:
    """Triangle function to calculate gable board-feet"""
    return .5 * power(leg, 2)
...
def main():
    ...
    long_walls = 2 * rectangle(width = building_x, length = building_y)
    short_walls = 2 * rectangle(width = building_z, length = building_y)
    roofs = 2 * rectangle(width = gable_leg, length = building_z)
    floor = rectangle(length = building_z, width = building_x)
    gables = 2 * triangle(leg = gable_leg)

    # Add all of the calculated areas to achieve the full
    # amount of lumber used
    total += long_walls + short_walls + roofs + gables + floor
